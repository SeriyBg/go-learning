...
func newDB() *PgStore {
	db, err := sql.Open("postgres", "user=goprojects database=goprojects sslmode=disable port=32773")
	if err != nil {
		panic(err)
	}
	return &PgStore{
...
func CreatePage(p *Page) (int, error) {
	var id int
	err := store.DB.QueryRow("INSERT INTO pages(title, content) VALUES($1, $2) RETURNING id", p.Title,
		p.Content).Scan(&id)
	return id, err
}
